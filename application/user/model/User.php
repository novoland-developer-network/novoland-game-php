...
class User extends Model
{
    ...
    public function login(string $username, string $password)
    {
        try {
            ...
            if (empty($userInfo)) {
                throw new Exception('用户不存在', 10101);
            }
            
            $res = password_verify($password, $userInfo['password']);
            if (!$res) {
                throw new Exception('密码错误', 10102);
            }
     ...
     */
    private static function issetUser(string $username): ?array
    {
    ...
    public function register(array $user): ?array
    {
        try {
            ...
            if ($isset) {
                throw new ErrorException('用户已存在', 10111);
            }
            ...
            if ($result <= 0) {
                throw new ErrorException('辰月出征，寸草不生', 10112);
            }
